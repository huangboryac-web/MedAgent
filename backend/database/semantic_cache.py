...
class VolatileSemanticCache:
    def __init__(self, ttl_seconds: int = 3600, cleanup_interval: int = 1800, max_workers: int = 4):
        ...
        self.executor = concurrent.futures.ThreadPoolExecutor(max_workers=max_workers)  # Pool for offloading

        self.index = faiss.IndexFlatL2(self.dimension)
        self.entries: List[Dict] = []

    async def check_cache(self, query: str, threshold: float = 0.35) -> Optional[str]:
        with self.lock:
        ...
        return await asyncio.get_running_loop().run_in_executor(self.executor, _check)

    async def update_cache(self, query: str, answer: str):
        def _prepare():
            try:
                clean_query = query.strip().lower()
                vector = np.array([self.embeddings.embed_query(clean_query)]).astype('float32')
                ...
                return vector, clean_query
            except Exception as e:
                logger.error(f"Cache update prep error: {e}")
                raise

        try:
            vector, clean_query = await asyncio.get_running_loop().run_in_executor(self.executor, _prepare)
